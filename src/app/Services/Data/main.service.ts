...
export class MainService{
  ...
  videosLoaded(){
    this.videoLoadedSubscription = this.completeService.videoIsLoaded().subscribe(
      (componentPage: string) =>{
        if(componentPage === 'main'){
          this.loadedVideos ++
          if(this.loadedVideos >= this.posts.length && this.isCanGetPost && this.getPostCount <= 1){
  ...
  canGetPostFun(){
    this.canGetPostSubscription = this.canGetPost.canGetPost().subscribe(
      (response: any)=> {
