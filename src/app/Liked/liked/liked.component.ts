...
export class LikedComponent implements OnInit, AfterViewInit,  OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.likedService.getPosts().subscribe(
      (posts: Post[]) =>{
        this.posts = posts
  ...
  ngAfterViewInit(): void {
    ...
    this.renderer.listen(this.container.nativeElement, 'scroll' , (e) =>{
      ...
      setTimeout(() => {  
        this.scroll.setScrolledHeight(this.container.nativeElement, "liked_scrolled_height")
      }, 1000);
    })
  ...
  scrollFun(){ 
    ...
    .subscribe(response =>{
      if(response >= 1){
        this.scroll.getScrolledHeight(this.container.nativeElement, "liked_scrolled_height") 
